...
join 
	Partnerships p 
ON 
	sp.PartnershipID = p.PartnershipID)
...
WITH HostRankCTE
     AS (SELECT HostName,
                ...
                Dense_rank()
                  OVER (
                    ORDER BY showcount DESC) AS HostRank
         FROM   Hosts)
SELECT *
FROM   HostRankCTE 